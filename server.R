...
shinyServer(
  ...
  function(input, output) {
    ...
    output$My_Example<- renderPlot({
      ...
      A1 <- data.frame(length1 = rbeta(1000000, prior_a+click_A1,peror_b+view_A1))
      B1 <- data.frame(length1= rbeta(1000000, prior_a+click_B1,peror_b+view_B1))
    ...
    output$Your_Work<- renderPlot({
      ...
      A <- data.frame(length = rbeta(1000000, prior_a+click_A,peror_b+view_A))
      B <- data.frame(length = rbeta(1000000, prior_a+click_B,peror_b+view_B))
